...
namespace NetTopologySuite.IO.GeoJSON4STJ.Test.Converters
{
    internal static class FeatureFactory
    {
        ...
        private static readonly IPrecisionModel PM = new PrecisionModel(PrecisionScale);
        private static readonly IGeometryFactory GF = NtsGeometryServices.Instance.CreateGeometryFactory(PM, 4326);
        private static readonly string[] RNDNames = new[] {"Random", "Zufall", "Hasard", "Caso", "Azar"};
        private static readonly IGeometry Bounds = GF.ToGeometry(new Envelope(-BoundX, BoundX, -BoundY, BoundY));
        ...
        public static IGeometry CreateRandomGeometry(in OgcGeometryType geometryType, in bool threeD = false)
        {
            switch (geometryType)
            {
        ...
        private static Coordinate CreateRandomCoordinate(in bool threeD = false)
        {
            ...
            var res = threeD
                ? new Coordinate(x, y, (double) RND.Next(0, BoundZ) / PrecisionScale)
                : new Coordinate(x, y);
            PM.MakePrecise(res);
            return res;
        }
        ...
        public static IPoint CreatePoint(in bool threeD)
        {
            return GF.CreatePoint(CreateRandomCoordinate(threeD));
        }

        public static ILineString CreateLineString(bool threeD)
        {
            ...
            for (int i = 0; i < pointsAt.Length; i++)
            {
                ...
                if (threeD)
                    pt = new Coordinate(pt.X + tmp.X, pt.Y + tmp.Y, start.Z + dz * pointsAt[i]);
                else
        ...
        private static IPolygon CreatePolygon(in bool threeD)
        {
            var centre = CreateRandomCoordinate(threeD);
            double radius = PM.MakePrecise(0.5 + 3d * RND.NextDouble());

            var polygon = (IPolygon)GF.CreatePoint(centre).Buffer(radius, 3);
            ...
            polygon = (IPolygon)polygon.Difference(hole);

            return (IPolygon)Bounds.Intersection(polygon);

        }

        private static IMultiPoint CreateMultiPoint(in bool threeD)
        {
            var geoms = new IPoint[RND.Next(2, 10)];
            for (int i = 0; i < geoms.Length; i++)
                geoms[i] = CreatePoint(threeD);
            return GF.CreateMultiPoint(geoms);
        }

        private static IMultiLineString CreateMultiLineString(in bool threeD)
        {
            var geoms = new ILineString[RND.Next(2, 10)];
            for (int i = 0; i < geoms.Length; i++)
                geoms[i] = CreateLineString(threeD);
            return GF.CreateMultiLineString(geoms);
        }

        private static IMultiPolygon CreateMultiPolygon(in bool threeD)
        {
            var geoms = new IPolygon[RND.Next(2, 10)];
            for (int i = 0; i < geoms.Length; i++)
                geoms[i] = CreatePolygon(threeD);
            return GF.CreateMultiPolygon(geoms);
        }

        private static IGeometry CreateGeometryCollection(in bool threeD)
        {
            var geoms = new IGeometry[RND.Next(2, 5)];
            for (int i = 0; i < geoms.Length; i++)
            {
                switch (RND.Next(2))
                {
        ...
        private static void SetCoordinate(ICoordinateSequence cs, int index, Coordinate coord)
        {
            cs.SetOrdinate(index, Ordinate.X, coord.X);
            cs.SetOrdinate(index, Ordinate.Y, coord.Y);
            if (!double.IsNaN(coord.Z))
                cs.SetOrdinate(index, Ordinate.Z, coord.Z);
        }
